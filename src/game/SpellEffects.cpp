...
void Spell::EffectSchoolDMG(uint32 effect_idx)
{
    if( unitTarget && unitTarget->isAlive())
    {
        switch(m_spellInfo->SpellFamilyName)
        {
            ...
            {
                // Judgement of Vengeance ${1+0.22*$SPH+0.14*$AP} + 10% for each application of Holy Vengeance or Blood Corruption on the target
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if(m_spellInfo->SpellFamilyFlags & 0x800000000000000LL)
            {
                ...
                for(Unit::AuraList::const_iterator itr = auraDiseases_FrostBlood.begin(); itr != auraDiseases_FrostBlood.end(); ++itr)
                {
                    ...
                    if( (*itr)->GetSpellProto()->Dispel == DISPEL_DISEASE && (*itr)->GetCasterGUID() == m_caster->GetGUID() )  
                        ++diseases;
                }
                ...
                m_damage += int32((m_spellInfo->EffectBasePoints[0]+1) * 15 / 100) * diseases;
                return;
            }
            // Corpse Explosion
...
void Spell::EffectHeal( uint32 /*i*/ )
{
    if( unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if(m_spellInfo->SpellFamilyFlags & UI64LIT(0x0200000000000000)){
            Unit::AuraList const& RejorRegr = unitTarget->GetAurasByType(SPELL_AURA_PERIODIC_HEAL);
